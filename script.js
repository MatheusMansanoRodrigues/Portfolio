...
/* ═══════════════════════════════════════════════════
   CONTACT FORM → Email
═══════════════════════════════════════════════════ */
const CONTACT_EMAIL = 'matheusmansano1@gmail.com';
...
document.getElementById('contact-form') && document.getElementById('contact-form').addEventListener('submit', function (e) {
    ...
    const subject = `[Portfólio] Contato de ${name}${company ? ` — ${company}` : ''}`;
    ...
    .then(data => {
        console.log('E-mail enviado via FormSubmit com sucesso:', data);
        ajaxSuccess = true;
        completeSubmission();
    })
    .catch(err => {
        console.warn('Erro ao enviar AJAX (usando fallback mailto):', err);
        ajaxSuccess = false;
        completeSubmission();
    });
    ...
    function completeSubmission() {
        ...
        setTimeout(() => {
            ...
            setTimeout(() => {
                if (!ajaxSuccess) {
                    // Open default mail client if AJAX fails
...
(function() {
    ...
    let particles = [];
    let sparkles = []; // Sparkles generated by mouse trail and clicks
    
    // Increased particle count and interaction radius
    const isMobile = window.innerWidth < 768;
    const maxParticles = isMobile ? 90 : 280;
    const connectionDist = 125;
    
    // Track mouse coordinates globally
    let mouse = { x: null, y: null, active: false };

    // Capture CSS Accent colors
    let accentColor = '#FF4D00';
    let creamColor = '#F0EBE1';
    
    function updateColors() {
        const rootStyles = getComputedStyle(document.documentElement);
        accentColor = rootStyles.getPropertyValue('--accent').trim() || '#FF4D00';
        creamColor = rootStyles.getPropertyValue('--cream').trim() || '#F0EBE1';
    }
    updateColors();
    ...
    class Particle {
        ...
        reset() {
            this.x = Math.random() * width;
            this.y = Math.random() * height;
            // Radius: some small nodes, some slightly larger
            this.radius = Math.random() * 2 + 0.8;
            // Original drift velocity vectors
            this.originalVx = (Math.random() - 0.5) * 0.45;
            this.originalVy = (Math.random() - 0.5) * 0.45;
            // Current velocity
            this.vx = this.originalVx;
            this.vy = this.originalVy;
            // Opacity: high for accent color, low for cream color
            this.isAccent = Math.random() < 0.25; // 25% orange nodes, 75% cream nodes
            this.alpha = this.isAccent ? Math.random() * 0.5 + 0.3 : Math.random() * 0.2 + 0.1;
        ...
        update() {
            ...
            // Handle gravity/attraction swarm field towards mouse cursor if active
            if (mouse.active && mouse.x !== null && mouse.y !== null) {
                ...
                if (dist < 220) {
                    ...
                    // Apply velocity friction only when interacting
                    this.vx *= 0.95;
                    this.vy *= 0.95;

                    // Accelerate particle away from cursor (repulsion physics)
            ...
            // Smoothly return to natural drifting velocity and base alpha when not targeted
            if (!isTargeted) {
            ...
            // Limit speeds to keep them fluid and manageable
            const speed = Math.sqrt(this.vx * this.vx + this.vy * this.vy);
            const maxSpeed = this.isAccent ? 3.0 : 2.0;
            if (speed > maxSpeed) {
        ...
        draw() {
            ...
            ctx.fillStyle = this.isAccent ? accentColor : creamColor;
    ...
    class Sparkle {
        constructor(x, y, isClick = false) {
            ...
            const speed = isClick ? (Math.random() * 3 + 1) : (Math.random() * 0.8 + 0.2);
            
            this.vx = Math.cos(angle) * speed;
            this.vy = Math.sin(angle) * speed;
            this.friction = isClick ? 0.95 : 0.98;
            this.alpha = 1;
            this.decay = Math.random() * 0.02 + (isClick ? 0.015 : 0.03);
        }
    ...
    // Particle Connections & Cursor Link
    function drawConnections() {
        ctx.lineWidth = 0.5;
        for (let i = 0; i < particles.length; i++) {
            const pi = particles[i];
            
            // Connect to mouse coordinates with a wider, brighter visual web
            if (mouse.active && mouse.x !== null && mouse.y !== null) {
            ...
            for (let j = i + 1; j < particles.length; j++) {
                ...
                if (dist < connectionDist) {
                    ...
                    ctx.strokeStyle = pi.isAccent || pj.isAccent ? accentColor : creamColor;
                    ctx.globalAlpha = alpha;
                    ctx.stroke();
                }
            }
    ...
    function loop() {
        ...
        // Update & Draw Sparkles (trail & clicks)
    ...
    function onMouseMove(e) {
        ...
        // Calculate speed to spawn sparkles
        ...
        if (dist > 8 && !isMobile) {
            ...
            for (let i = 0; i < count; i++) {
                ...
                sparkles.push(new Sparkle(px, py, false));
            }
        }
